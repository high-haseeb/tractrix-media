        ...
        <div className='w-full flex flex-col lg:gap-4 gap-2'>
            {
                variants.map((variant, idx) =>
                    <div
                        key={idx}
                        className={`w-full rounded-full cursor-pointer transition-all flex items-center justify-between px-8 py-2 lg:py-3 border border-black ${activeVariantName === variant.name ? "bg-black/10 text-black" : "bg-none text-black/80 border-black/50 "} text-sm lg:text-base`}
            ...
            }
            {
                activeFinanceOption === "finance" && (
                    <div className='flex flex-col w-full items-center justify-center gap-2 mt-2'>
                        <div className='text-xs lg:text-base font-light text-gray-600'>$3,000 down, 6% APR, 72 months</div>
                        <Link className="text-sm lg:text-base font-semibold text-gray-500 border-b border-b-gray-500 leading-none" href={'/edit-terms'}> Edit Terms</Link>
                    </div>
                )
            }
        </div>
